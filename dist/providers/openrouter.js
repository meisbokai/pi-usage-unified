 ...
 */
export function parseOpenRouterKey(parsed) {
    const data = parsed?.data ?? parsed;
    const usage = toFinite(data?.usage);
    const limit = toFinite(data?.limit);
    const result = {};
    if (usage !== undefined)
        result.keySpend = usage;
    if (limit !== undefined)
        result.keyLimit = limit;
 ...
 */
export function parseOpenRouterCredits(parsed) {
...
async function fetchOpenRouter(ctx) {
...
function renderStatus(data, theme) {
    const main = data.keyUsagePercent !== undefined
        ...
        : data.keyLimit !== undefined
            ...
            : fg(theme, "muted", "n/a");
    const balance = data.creditBalance !== undefined
        ? `${fg(theme, "dim", " bal ")}${colorForCredit(data.creditBalance, theme)(money(data.creditBalance))}`
        : "";
    const spend = data.keySpend !== undefined ? `${fg(theme, "dim", " spent ")}${fg(theme, "dim", money(data.keySpend))}` : "";
