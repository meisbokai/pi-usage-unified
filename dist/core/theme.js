...
export function fg(theme, color, text) {
 ...
 */
export function colorForPercentage(percentage, theme, thresholds = loadColorThresholds()) {
    if (percentage >= thresholds.percentage.critical)
        return (s) => fg(theme, "error", s);
    if (percentage > thresholds.percentage.warning)
        return (s) => fg(theme, "warning", s);
 ...
 * - error (red) when ≤ critical threshold
 */
export function colorForCredit(credit, theme, thresholds = loadColorThresholds()) {
    if (credit <= thresholds.credit.critical)
        return (s) => fg(theme, "error", s);
    if (credit < thresholds.credit.warning)
        return (s) => fg(theme, "warning", s);
    return (s) => fg(theme, "accent", s);
}
