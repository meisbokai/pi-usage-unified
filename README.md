...
- **DeepSeek** — `GET api.deepseek.com/user/balance`. Renders every currency wallet from `balance_infos` (`currency`, `total_balance`, `granted_balance`, `topped_up_balance`, `discounted_balance`) as credit balances — one amount per wallet, not a percentage — each wallet coloured by its own balance. An `is_available: false` balance renders every wallet in the error colour. The legacy single `balance` object (`is_sufficient`) is still accepted.
