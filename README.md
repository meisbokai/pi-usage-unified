...
- **DeepSeek** — `GET api.deepseek.com/user/balance`. Renders every currency wallet from `balance_infos` (`currency`, `total_balance`, `granted_balance`, `topped_up_balance`) as credit balances — one amount per wallet, not a percentage — with the status colour driven by the least-funded wallet. The legacy single `balance` object (`is_sufficient`) is still accepted.
