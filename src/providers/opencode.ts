...
async function probeOpencode(ctx: FetchContext): Promise<OpencodeUsageData> {
  ...
  if (!apiKey) {
    throw new UsageError(
      "OpenCode Go key not found. Set OPENCODE_API_KEY or log in via /login.",
      "noauth",
    );
  }
  ...
  if (response.status === 429) throw new UsageError("OpenCode rate limited", "ratelimited");
  if (!response.ok) {
    throw new UsageError(`OpenCode probe HTTP ${response.status}`, `http${response.status}`);
  }

  const headers = Object.fromEntries(response.headers.entries()) as Record<string, string>;
  const rolling =
    numberHeader(headers, "x-opencode-rolling-usage-percent") ??
    numberHeader(headers, "x-opencode-rolling-used-percent") ??
    numberHeader(headers, "x-rolling-usage-percent");
  const weekly =
    numberHeader(headers, "x-opencode-weekly-usage-percent") ??
