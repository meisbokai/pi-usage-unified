...
import { formatDurationFromNow, pct, roundPercent } from "../core/format.js";
...
export function cursorDbPaths(home: string = homedir()): string[] {
  return [
...
export async function loadCursorAuth(): Promise<CursorAuth | undefined> {
...
export async function refreshCursorAccessToken(refreshToken: string): Promise<string | undefined> {
 ...
 */
export function resolveCursorPercentages(input: RawPercentages): {
  autoPercentUsed: number;
  apiPercentUsed: number;
  totalPercentUsed: number;
} {
  ...
  let total: number | undefined = Number.isFinite(Number(input.totalPercentUsed))
...
export function getTeamMemberSpendList(response: any): any[] {
  return response?.teamMemberSpend ?? response?.spend ?? [];
}

/** Pick the matching member by email, or the only member, or undefined. */
export function pickTeamMember(members: any[], preferredEmail?: string): any | undefined {
...
async function fetchCursorDashboardWithToken(
  accessToken: string,
  email?: string,
): Promise<CursorUsageData> {
...
export async function fetchCursorDashboardUsage(): Promise<CursorUsageData> {
  const auth = await loadCursorAuth();
  if (!auth?.accessToken) {
    throw new UsageError(
      "Cursor session not found. Open Cursor Desktop or set CURSOR_ADMIN_API_KEY.",
      "noauth",
    );
  }
  try {
    ...
    if (error instanceof UsageError && error.code === "http401" && auth.refreshToken) {
      ...
      if (refreshed) return await fetchCursorDashboardWithToken(refreshed, auth.email);
    }
    throw error;
  }
}

async function fetchCursor(ctx: FetchContext): Promise<CursorUsageData> {
  const mode = process.env.CURSOR_USAGE_MODE?.toLowerCase?.();
  const preferredEmail =
    process.env.CURSOR_USAGE_EMAIL?.trim() || (await loadCursorAuth())?.email;
  const envAdminKey =
  ...
  if (mode === "admin") {
    if (!adminKey) throw new UsageError("CURSOR_ADMIN_API_KEY is required for admin mode", "noauth");
...
function renderStatus(data: CursorUsageData, theme: Theme): string {
...
function formatDetails(data: CursorUsageData): string {
  return [
    "Cursor usage",
...
export const cursorProvider: UsageProvider<CursorUsageData> = {
  id: "cursor",
  label: "Cursor",
