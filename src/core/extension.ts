...
import { createCache, type UsageCache } from "./cache.js";
...
function clearLegacyStatuses(ctx: ExtensionContext): void {
  if (process.env[CLEAR_LEGACY_ENV] === "0") return;
  for (const key of LEGACY_STATUS_KEYS) {
    ctx.ui.setStatus(key, undefined);
  }
}

function defaultRenderError(providerId: string, error: unknown, theme: ExtensionContext["ui"]["theme"]): string {
 ...
 * ```
 */
export function createUnifiedExtension(registry: ProviderRegistry): (pi: ExtensionAPI) => void {
  return function extension(pi: ExtensionAPI): void {
    const cache: UsageCache = createCache();
    ...
    async function showUsage(args: string | undefined, ctx: ExtensionContext): Promise<void> {
